...
|     ---     |     ---   |     ---   |             ---                 |
| 1.0 | 23/04/2026 | C. Diego, R. Daniel, V. Liam, P. Gerardo | Se duplico el diseño/esqueleto del informe, frontend, backend, landing page y json server |
| 1.1 | 25/04/2026 | C. Diego, R. Daniel, V. Liam, P. Gerardo | Se verificaron los errores y cambios de entregas anteriores |
| 1.2 | 30/04/2026 | C. Diego, R. Daniel, V. Liam, P. Gerardo | Se verifico el funcionamiento del frontend y backend |
...
- [Capítulo I: Introducción](#capítulo-i-introducción)
  ...
  - [1.2. Solution Profile](#12-solution-profile)
    - [1.2.1 Antecedentes y problemática](#121-antecedentes-y-problemática)
        ...
        - [**¿Cuándo ocurre el problema? (When)**](#cuándo-ocurre-el-problema-when)
        - [**¿Dónde ocurre el problema? (Where)**](#dónde-ocurre-el-problema-where)
        - [**¿A quién afecta el problema? (Who)**](#a-quién-afecta-el-problema-who)
        - [**¿Por qué sucede el problema? (Why)**](#por-qué-sucede-el-problema-why)
        - [**¿Cómo sucede el problema? (How)**](#cómo-sucede-el-problema-how)
        - [**¿Cuán grande es el impacto de este problema? (How much)**](#cuán-grande-es-el-impacto-de-este-problema-how-much)
...
- [Interactividad](#interactividad)
  - [4.1.2. Web Style Guidelines](#412-web-style-guidelines)
    ...
    - [Animaciones suaves](#animaciones-suaves)
  ...
  - [4.6. Domain-Driven Software Architecture.](#46-domain-driven-software-architecture)
    ...
    - [4.6.4. Software Architecture Components Diagrams.](#464-software-architecture-components-diagrams)
      ...
      - [2. Assignment Service](#2-assignment-service)
...
- [Capítulo V: Product Implementation, Validation \& Deployment](#capítulo-v-product-implementation-validation--deployment)
  ...
  - [**5.5. Acuerdo de Servicio - SaaS.**](#55-acuerdo-de-servicio---saas)
    - [1. Descripción del Alcance del Servicio](#1-descripción-del-alcance-del-servicio)
    - [2. Disponibilidad del Servicio (Uptime)](#2-disponibilidad-del-servicio-uptime)
    - [3. Soporte Técnico y Tiempos de Respuesta](#3-soporte-técnico-y-tiempos-de-respuesta)
    - [4. Seguridad, Privacidad y Control de Acceso](#4-seguridad-privacidad-y-control-de-acceso)
    - [5. Responsabilidades del Cliente](#5-responsabilidades-del-cliente)
    - [6. Actualizaciones y Mantenimiento Evolutivo](#6-actualizaciones-y-mantenimiento-evolutivo)
...
- [Capítulo VII: DevOps Practices](#capítulo-vii-devops-practices)
  ...
  - [7.2. Continuous Delivery.](#72-continuous-delivery)
    ...
    - [Practices (Prácticas)](#practices-prácticas)
      - [Feature Branching y Pull Requests](#feature-branching-y-pull-requests)
      - [Continuous Integration](#continuous-integration)
      - [Automated Testing](#automated-testing)
      - [Validation in Development Environment](#validation-in-development-environment)
    ...
    - [Validación en Entorno de Desarrollo](#validación-en-entorno-de-desarrollo)
    - [Pruebas Automatizadas](#pruebas-automatizadas)
    - [Revisión Manual del Código](#revisión-manual-del-código)
    - [Monitoreo y Retroalimentación](#monitoreo-y-retroalimentación)
  - [7.3. Continuous deployment.](#73-continuous-deployment)
    - [7.3.1. Tools and Practices.](#731-tools-and-practices)
    - [Tools (Herramientas)](#tools-herramientas)
    - [Practices (Prácticas)](#practices-prácticas-1)
      - [Feature Branching](#feature-branching)
      - [Commit-Based Deployment](#commit-based-deployment)
      - [Automated Testing Before Deployment](#automated-testing-before-deployment)
      - [Continuous Deployment Workflow](#continuous-deployment-workflow)
      - [Version Control and Recovery](#version-control-and-recovery)
    - [7.3.2. Production Deployment Pipeline Components.](#732-production-deployment-pipeline-components)
    - [Componentes del Pipeline de Integración Continua (GitHub)](#componentes-del-pipeline-de-integración-continua-github)
      - [1. Control de Versiones](#1-control-de-versiones)
      - [2. Integración de Cambios](#2-integración-de-cambios)
      - [3. Validación Automática](#3-validación-automática)
      - [4. Seguimiento y Recuperación de Versiones](#4-seguimiento-y-recuperación-de-versiones)
    - [Componentes del Pipeline de Pruebas del Frontend (Angular)](#componentes-del-pipeline-de-pruebas-del-frontend-angular)
      - [1. Compilación del Frontend](#1-compilación-del-frontend)
      - [2. Ejecución de Pruebas Unitarias](#2-ejecución-de-pruebas-unitarias)
...
|:--------------------------------------------------------------------------------------------------------------------------------------------------------|:--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Reconoce responsabilidad ética y profesional en situaciones de ingeniería de software.                                                                  | **Cacho Seminario, Diego Alonso**<br>TB1: Para el desarrollo de la TB1, aporte creando los documentos de exposicion, reporte de participación y además prepare el reporte principal con los items anteriores para la entrega.<br><br>TP: Para el desarrollo del TP, aporte con el punto de acuerdo de servicios SaaS y realice las correcciones de la entrega pasada.<br><br>TB2: Para el desarrollo de la TB2, aporté en la elaboración parcial del Capítulo VIII definiendo las tarjetas de experimentos, las historias de usuario To-Be y la actualización del Acuerdo de Servicio SaaS, asegurando el compromiso con la privacidad de los datos.<br><br>**Villugas Jeronimo, Liam Anderson**<br>TB1: Para el desarrollo de la TB1, aporte con el informe inicial del trabajo.<br><br>TP: Para el desarrollo del trabajo parcial, aporté con el soporte técnico en la elaboración del informe inicial del proyecto, asegurando que los contenidos relacionados con la implementación, arquitectura y pruebas reflejaran un compromiso ético con la calidad, la transparencia y la confiabilidad del software. Esto incluye la documentación de buenas prácticas y la justificación de decisiones técnicas orientadas a minimizar riesgos en el sistema SafeWork. <br><br> TB2: En el marco de la TB2, participé activamente en la construcción del Capítulo VIII, contribuyendo en la definición de las tarjetas de experimentos, la redacción de las historias de usuario del escenario To-Be y la revisión del Acuerdo de Servicio SaaS. Mi trabajo estuvo orientado a garantizar que cada uno de estos entregables reflejara el compromiso con la privacidad y protección de los datos de los usuarios.<br><br>**Ruiz Huisa, Daniel Elias**<br>TB1: Para el desarrollo del presente trabajo, revise la correcta regresion de las versiones deployadas del proyecto, a versiones estables que puedan adaptarse a otros entornos de despliegue.<br><br>TP:Para el desarrollo del trabajo, entendi la importancia del backlog y su cumplimiento para el desarrollo de un proyecto Siendo principalmente mi tarea, en asegurarme que en la version desplegada, se cumpla cada uno de estos items<br><br>TB2:En esta entrega me encargue de racionalizar los experimentos para establecer una linea de desarrollo posible para cada escenario planteado, buscando alternativas de solucion para cada uno a de las cards<br><br>**Palacin Lazo, Gerardo Valentin**<br>TB1: Para el desarrollo del trabajo, logre participar en el informe apoyando en la verificación y despliegue de la información requerida de los capítulos a entregar. <br> TP: Para el desarrollo del trabajo parcial he sido encargado de verificar y validar el product backlog actualizado con la metodología de CI e CD para presentar pruebas que reflejan nuestro compromiso de seguir mejorando. <br> TB2: Durante esta entrega apliqué la metodologia de XDPD aplicado al to-be product backlog dentro de un entorno experimental registrando los datos en experiment cards para después generando nuestras hipótesis. | En la TB1, los integrantes del equipo revisaron el avance general de la entrega anterior y realizaron cambios para el funcionamiento aceptable de la aplicación. <br><br>En el TP, los integrantes del equipo realizaron las correcciones brindadas en la entrega anterior, además se realizó el despliegue del Landing Page, Frontend y Backend.<br><br>En la TB2, el equipo planificó los experimentos y métricas (Capítulo VIII) garantizando la evaluación objetiva y segura de las nuevas funcionalidades.                         |
| Emite juicios informados considerando el impacto de las soluciones de ingeniería de software en contextos globales, económicos, ambientales y sociales. | **Cacho Seminario, Diego Alonso**<br>TB1: Como primera entrega utilice las herramientas de Github y Canva para poder redactar y crear la información necesaria para el trabajo de SafeWork.<br><br>TP: Para el desarrollo del TP, nuevamente se utilizaron las herramientas anteriormente mencionadas adicionando el uso de Selenium para las pruebas del Frontend.<br><br>TB2: En esta entrega propuse y justifiqué el uso de métricas de negocio y herramientas de auditoría técnica (como Google Lighthouse) para evaluar el impacto de las mejoras en accesibilidad, productividad y rendimiento.<br><br>**Villugas Jeronimo, Liam Anderson**<br>TB1: Como primera entrega utilice herramientas de github y markdown para el informe para SafeWork.<br><br>TP: Como entrega del trabajo parcial (TP), utilicé herramientas como GitHub (control de versiones y colaboración) y Markdown (para la redacción del informe técnico del proyecto SafeWork). Además, contribuí con la implementación de pruebas unitarias automatizadas, y pruebas de interfaz con Selenium, con el objetivo de garantizar la fiabilidad del sistema. <br><br>TB2: En esta etapa, planteé y fundamenté la incorporación de métricas orientadas al negocio, así como el uso de herramientas de auditoría técnica , con el fin de medir el efecto real de las mejoras implementadas en términos de accesibilidad, productividad y rendimiento del sistema. <br><br>**Ruiz Huisa, Daniel Elias**<br>TB1: Como orimers entrega, aproveché la herramienta git, oara identificar los puntos de desarrollo clave que nos permitan desplegar nuevamente el aplicativo en posteriores entregas.<br><br>TP:Me encargue de revisar el despliegue y su correcta implementacion, para asegurar que este actualizada a la ultima versiion del proyecto, la cual cumple con todos los features que presetntamos en el backlog. <br><br>TB2: Para la entrega, me encargue de revisar ela viabilidad d cada tarjeta de experimentos a nivel tecnico e implementacion.<br><br>**Palacin Lazo, Gerardo Valentin**<br>TB1: Mediante la herramienta de github y commits pude documentar mi avance en el informe requerido para esta entrega. <br> TP:Durante esta entrega utilizé herramientas de colaboración y de pruebas de flujo de la página frontend como Selenium. Evaluando el impacto en posibles usuarios y registrando los datos. <br> TB2: Para la entrega de este trabajo, redefiní las tarjetas de experimentación para su correcta presentación y la ayuda de herramientas para poder capturar datos que verifiquen mis tarjetas de hipótesis.                                                                                                                                                                                                                                 | Durante la TB1, el equipo logró utilizar las herramientas apropiadas para la entrega, se uso GitHub como repositorio principal para el proyecto, Google Docs para el reporte de participación y Canva como herramienta para crear la presentación de exposición. <br><br>En el TP, el equipo utilizo nuevas herramientas para la entrega como Selenium, Vercel o Render.<br><br>En la TB2, se establecieron KPIs y se seleccionaron herramientas de análisis y auditoría para medir empíricamente el impacto de las mejoras propuestas. |
...
|----------------------------|-------------------------------------|----------------------|---------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ![imgs](imgs/DiegoC.png)   | **Diego Alonso Cacho Seminario**    | u202223990           | Ingeniería de Software | Habilidades: Soy estudiante de Ingeniería de Software cursando el 8vo ciclo de mi carrera en la UPC y tengo 21 años. Me considero una persona tranquila y diligente, intentó realizar mis tareas y trabajos lo antes posible para evitar contratiempos en un futuro, especialmente si son actividades que consumen mucho tiempo. Como miembro de equipo buscaré ayudar a mis compañeros cuando lo necesiten, realizando además mis entregas lo más temprano posible. Habilidades en C++, C#, Python, Unity 2D/3D, html/css/js.                                                    |
| ![imgs](imgs/GerardoP.png) | **Gerardo Valentin Palacin Lazo**   | u20211c201           | Ingeniería de Software | Soy estudiante de Ingeniería de Software cursando el 6to ciclo. Me fascina el aporte de las soluciones tecnológicas de la actualidad y como ayudan a la gente en tareas cotidianas aprovechando más el tiempo en otros asuntos más importantes. Es por eso que me esfuerzo por comprender este mundo desde la responsabilidad, diligencia y honestidad para ser un profesional que yo considere de respetar, me ayudo de la tecnología actual y de mis compañeros de trabajo para mejorar mi aprendizaje. mis habilidades principales para trabajar son c++, Java, html, css, js. |
| ![imgs](imgs/Daniel.jpeg)     | **Daniel Elias Ruiz Huisa**                 | u202210764     | Ingeniería de Software |     Estudiante de Ingeniería de Software. Me intereso por el desarrollo web y la evolucion de tecnologias como los nuevos agentes AI. Tengo conocimientos en Frameworks orientados a node.js como Astro, Vue y Angular. Domino lenguajes como python, C++ y typescript. Soy una persona responsable que busca siempre generar un ambiente sano y agradable para todos.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ![imgs](imgs/FotoLiam.png) | **Liam Anderson Villugas Jeronimo** | u20221634            | Ingeniería de Software | Soy Estudiante de Ingienieria de Software en la Universidad Peruana de Ciencias Aplicadas (UPC), Estoy cursando actualmente el 7mo ciclo de la carrera.Me atrae mucho el desarrollo de aplicaciones y el uso de las bases de datos. Me considero una persona en Aprendizaje constante e intento mantener la tranquilidad bajo prension al trabajar.                                                                                                                                                                                                                               |
...
![Problema 1](imgs/cap6/ux_audit/problem-1.png)
...
![Problema 2](imgs/cap6/ux_audit/problem-2.png)
...
![Problema 4](imgs/cap6/ux_audit/problem-4.png)
...
![Problema 5](imgs/cap6/ux_audit/problem-5.png)
...
![Problema 6](imgs/cap6/ux_audit/problem-6.png)
...
![Problema 7](imgs/cap6/ux_audit/problem-7.png)
...
![Problema 8](imgs/cap6/ux_audit/problem-8.png)
...
![Problema 9](imgs/cap6/ux_audit/problem-9.png)
...
![Problema 10](imgs/cap6/ux_audit/problem-10.png)
...
Tras la finalización del **Sprint To-Be** y el despliegue de las nuevas funcionalidades en el entorno de pruebas, se procedió a recopilar y analizar la retroalimentación de los usuarios mediante un formulario de Google Forms aplicado a una muestra total de **20 usuarios** (compuesta por personal SST y Trabajadores). Adicionalmente, se utilizaron herramientas de auditoría técnica (Google Lighthouse) para respaldar objetivamente las métricas de rendimiento.

### 8.4.1. Analysis and Interpretation of Results

A continuación, se presentan los resultados obtenidos del formulario y las auditorías técnicas, junto con la interpretación y la decisión de negocio para cada hipótesis planteada.
...
* **Opciones:** 20-30 años / 31-40 años / 41-50 años / 51-60 años.
 ![Distribución de edades](imgs/cap8/pregunta1form.png)
...
* **Opciones:** Diariamente / Semanalmente / Mensualmente.
 ![Frecuencia de uso](imgs/cap8/pregunta2form.png)
...
**Hipótesis 1 (US01 - Integración de Mapa Interactivo)**
* **Pregunta 3:** ¿El mapa interactivo integrado en la plataforma le permitió ubicar el incidente de manera más rápida en comparación a hacer clic en un enlace externo?
    * **Opciones:** Sí / No.
     ![Resultado Mapa Interactivo](imgs/cap8/pregunta3form.png)
* **Interpretación:** El 90% (18 de 20 encuestados) respondió afirmativamente. Este resultado supera nuestra expectativa inicial de mejora, confirmando que la integración nativa reduce la carga cognitiva y optimiza el tiempo de respuesta del usuario.
* **Conclusión:** **HIPÓTESIS VALIDADA.** La funcionalidad ha demostrado su valor y se aprueba su paso a **Producción**.

**Hipótesis 2 (US02 - Cambio de Estado Rápido)**
* **Pregunta 4:** ¿Considera que los botones de acción rápida en la lista principal agilizan la gestión y actualización de los reportes?
    * **Opciones:** Totalmente de acuerdo / De acuerdo / En desacuerdo / Totalmente en desacuerdo.
     ![Resultado Acción Rápida](imgs/cap8/pregunta4form.png)
* **Interpretación:** El 85% de los encuestados seleccionó "Totalmente de acuerdo". Esto confirma la creencia de que reducir el número de clics mediante acciones directas impacta positivamente en la productividad operativa.
* **Conclusión:** **HIPÓTESIS VALIDADA.** Se aprueba su despliegue a **Producción**.

**Hipótesis 3 (US03 - Optimización Visual y Accesibilidad)**
...
**Hipótesis 4 (US04 - Gráfico de Tasa de Reclamos)**
* **Pregunta 6:** ¿Qué tan útil le resulta la visualización de la gráfica de "Tasa de Reclamos" en el panel principal para la toma de decisiones preventivas?
    * **Opciones:** Muy útil / Algo útil / Poco útil / Nada útil.
     ![Resultado Gráfico Reclamos](imgs/cap8/pregunta6form.png)
* **Interpretación:** El 95% (19 de 20 encuestados) la calificó como "Muy útil". Esto supera ampliamente el objetivo del 80% propuesto en nuestra tarjeta de experimento, demostrando que los datos consolidados ahorran tiempo analítico al usuario.
* **Conclusión:** **HIPÓTESIS VALIDADA.** La gráfica se integrará definitivamente en **Producción**.

**Hipótesis 5 (US05 - Filtros Rápidos de Navegación)**
* **Pregunta 7:** ¿Los botones de filtros rápidos (ej. "Críticos", "Abiertos") en la página de inicio hicieron que su búsqueda de información fuera más intuitiva?
    * **Opciones:** Sí / No.
     ![Resultado Filtros Rápidos](imgs/cap8/pregunta7form.png)
* **Interpretación:** El 60% de los encuestados respondió afirmativamente. Aunque es una mayoría, el feedback del formulario indicó que la UI actual de los botones ocupa demasiado espacio y puede resultar confusa al filtrar por múltiples criterios. No se logró la meta de satisfacción del 80% (o el 20% de mejora sobre el estado anterior).
